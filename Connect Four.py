...
def generateInitialBlankBoard():
    tempArray = [] # Used to create [ 0, 0, 0...]
    for i in range(1, 13):
        tempArray = []
        for j in range(1, 9):
            tempArray.append(0)
        ...
        for j in range(0, len(tempColoursArray[x])):
            if (j + 3) < len(tempColoursArray[x]):
                if (tempColoursArray[x][j] != 0) and (
                        tempColoursArray[x][j] == tempColoursArray[x][j + 1]
                ) and (tempColoursArray[x][j] == tempColoursArray[x][j + 2]
                       ) and (tempColoursArray[x][j]
                              == tempColoursArray[x][j + 3]):
    ...
    for x in range(1, len(tempColoursArray)):
        for j in range(0, len(tempColoursArray[x])):
            if (x) < len(tempColoursArray) - 3:
                if (tempColoursArray[x][j] != 0) and (
                        tempColoursArray[x][j] == tempColoursArray[x + 1][j]
                ) and (tempColoursArray[x][j] == tempColoursArray[x + 2][j]
                       ) and (tempColoursArray[x][j]
                              == tempColoursArray[x + 3][j]):
        ...
        for j in range(0, len(tempColoursArray[x])):
            if ((x) < len(tempColoursArray) - 3) and (
                ...
                if (tempColoursArray[x][j] !=
                        0) and (tempColoursArray[x][j]
                                == tempColoursArray[x + 1][j + 1]) and (
                                    tempColoursArray[x][j]
                                    == tempColoursArray[x + 2][j + 2]) and (
                                        tempColoursArray[x][j]
                                        == tempColoursArray[x + 3][j + 3]):
...
def makeBoard():
    generateInitialBlankBoard()
    row = 1
    column = 0
    root.button2 = []
    for i in range(0, 96):
        root.button2.append(Button(root, image=blankButtonImage,
                                   borderwidth=0))
